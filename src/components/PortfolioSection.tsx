...
const projects: Project[] = [
  ...
  },
  {
    ...
    media: [
      {
        type: "video",
        src: "https://player.mux.com/vLaqjKFJz2Opq5kOumK00TmsHQHRM00eyv01502M00sg9VWQ?metadata-video-title=Villa_Ormaneto&video-title=Villa_Ormaneto&thumbnail-time=2&accent-color=%23a98860",
  ...
  },
  {
    ...
    media: [
      {
        type: "video",
        src: "https://player.mux.com/MFVl8gTZwV02ZzjbeAxYorTIcRk7bwJeSXeB00NaHBGN4?metadata-video-title=Castelmani_Storia&video-title=Castelmani_Storia&thumbnail-time=4&accent-color=%23a98860",
                    ...
                    <CarouselContent className="-ml-0">
                      {selectedMedia.map((media, index) => (
                        <CarouselItem key={`selected-project-media-${selected.projectIndex}-${index}`} className="pl-0">
                          <div className="flex h-[min(64dvh,calc(100dvh-8.5rem))] min-h-[220px] w-full items-center justify-center overflow-hidden rounded-[22px] border border-border bg-background/70 sm:h-[min(72dvh,calc(100dvh-14rem))]">
                            {media.type === "image" ? (
                              <img
                                src={media.src}
